...
#define item_at(arr_itm, idx) \
    _item_at(arr_itm, (idx < 0 ? (arr_itm).value.array->count + idx : idx))

#define _item_at(arr_itm, idx) \
    (*(json_Item *)array__item_ptr((arr_itm).value.array, idx))
...
#define item_of(obj_itm, key) \
    json_item_or_error(map__get((obj_itm).value.object, (void*)(key)))
...
#define copy_str_item(str) \
    ((json_Item){ .type = item_string, .value.string = strdup(str) })

#define wrap_str_item(str) \
    ((json_Item){ .type = item_string, .value.string = (char *)str })

#define new_arr_item() \
    ((json_Item){ .type = item_array,  .value.array = json_array() })
...
// This macro exists to make it easier to use the string type as a
// general-purpose pointer container for C-to-C communications.
...
//
// In the future, an array can also end with ,* to mean 0 or more items are
// allowed; otherwise an exact match is expected. For example [#,#] expects
// exactly two numbers, while [#,#,*] expects two numbers either alone or
